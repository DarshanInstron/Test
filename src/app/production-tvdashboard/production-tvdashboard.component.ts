...
@Component({
  selector: 'app-production-tvdashboard',
  templateUrl: './production-tvdashboard.component.html',
  styleUrls: ['./production-tvdashboard.component.scss']
})
export class ProductionTvdashboardComponent implements OnDestroy,OnInit {
  ...
  constructor(private router: Router, public _OeeDashboardService: OeeDashboardService, changeDetectorRef: ChangeDetectorRef, 
    private cd: ChangeDetectorRef,public _CommonSettingPageService: CommonSettingService, private _GlobalConstantsService: GlobalConstantsService,
    public _ValidationerrormessagesService:ValidationerrormessagesService) {
  ...
  ngOnInit(): void {
    ...
    this.LeftSideTreeviewHeight = (h - 95) + "px";
    //this.GetDashboardCardDetailsInfo("10"); 
    this.prodContentHeight = (h-160)+"px";
    //this.GetDashboardCardDetailsInfo("10");
  ...
  GetDashboardCardDetailsInfo(LevelId: any,IsWithSlider:any, SequenceID:any) {
    ...
    this._OeeDashboardService.getOeeDashboardDetailsListByLevelIdSliderSequence('', '','', LevelId, IsWithSlider, SequenceID,this.IsPageLoad,"1").subscribe((response: any) => {
      //this._ValidationerrormessagesService.CheckErrorMsgIsFoundOrNotForSession(response);
      if(this.IsPageLoad=="1" || this.TempLevelId!=LevelId||this.TempSlider!=IsWithSlider||this.TempSequenceID!=SequenceID){
